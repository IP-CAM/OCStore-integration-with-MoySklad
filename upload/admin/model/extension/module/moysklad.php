...
class ModelExtensionModuleMoysklad extends Model
{
    ...
    public function getAttributeGroupByName($attribute_group_name) {
		$query = $this->db->query("SELECT * 
                                   FROM " . DB_PREFIX . "attribute_group_description AS agd
                                   WHERE agd.language_id = '" . (int)$this->config->get('config_language_id') . "' 
                                   AND agd.name = '" . $this->db->escape($attribute_group_name) . "'");
    ...
    public function getProductIdByExternalCode($external_code)
    {
        $sql = "SELECT * 
                ...
                WHERE p.model = '" . $this->db->escape($external_code) . "'";
    ...
    {
        $sql = "UPDATE `" . DB_PREFIX . "product` AS p 
                SET p.quantity = '" . (int)$quantity . "' 
                WHERE p.model = '" . $this->db->escape($product_external_code) . "'";
    ...
    {
        $sql = "UPDATE `" . DB_PREFIX . "product_option_value` AS pov 
                SET pov.quantity = '" . (int)$quantity . "' 
                WHERE pov.model = '" . $this->db->escape($option_external_code) . "'";
